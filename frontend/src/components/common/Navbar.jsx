...
import learnlang from "../../assets/Logo/learnlang.png";
...
const Navbar = () => {
  ...
  return (
    <nav
      className={`z-[10] flex h-14 w-full items-center justify-center bg-white absolute top-0 mb-10 text-white-700 translate-y-0 transition-all `}
    >
      ...
      <div className="flex w-11/12 max-w-maxContent items-center justify-between ">
        {/* logo */}
        <Link to="/">
          <img src={learnlang} width={160} height={42} loading="lazy" />
        </Link>

        {/* Nav Links - visible for only large devices*/}
        <ul className="hidden sm:flex gap-x-6 text-white-25">
                ...
                <div
                  className={`group relative flex cursor-pointer items-center gap-1 ${
                    matchRoute("/catalog/:catalogName")
                      ...
                      : "text-white-25 rounded-xl p-1 px-3"
                  }`}
                >
                  <p>{link.title}</p>
                  <MdKeyboardArrowDown />
                  ...
                  <div
                    className="invisible absolute left-[50%] top-[50%] z-[1000] flex w-[200px] translate-x-[-50%] translate-y-[3em] 
                                                    flex-col rounded-lg bg-richblack-5 p-4 text-white-900 opacity-0 transition-all duration-150 group-hover:visible 
        ...
        <div className="flex gap-x-4 items-center">
          {user && user?.accountType !== "Instructor" && (
            <Link to="/dashboard/cart" className="relative">
              <AiOutlineShoppingCart className="text-[2.35rem] text-white-5 hover:bg-richblack-700 rounded-full p-2 duration-200" />
              {totalItems > 0 && (
          ...
          {token === null && (
            <Link to="/login">
              {/* <button className='border border-richblack-700 bg-richblack-800 px-[12px] py-[8px] text-white-100 rounded-md focus:outline-8 outline-yellow-50'> */}
              <button
                className={` px-[12px] py-[8px] text-white-100 rounded-md 
                                 ${
                                   matchRoute("/login")
                                     ? "border-[2.5px] border-yellow-50"
                                     : "border border-richblack-700 bg-#ffa500"
                                 } `}
              >
                Log in
              </button>
          ...
          {token === null && (
            <Link to="/signup">
              {/* <button className='border border-richblack-700 bg-#ffa500 px-[12px] py-[8px] text-white-100 rounded-md'> */}
              <button
                className={` px-[12px] py-[8px] text-white-100 rounded-md 
                                 ${
                                   matchRoute("/signup")
